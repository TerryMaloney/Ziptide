# HANDOFF ARCHIVE — entries through 2026-07-28

Archived 2026-07-28 per `docs/FAST_LANE.md` §4.1. `docs/HANDOFF.md` had grown to 3,155 lines /
138 entries, and the laws require every operator to read it at session start — a read-in tax paid
every session by every operator, on top of CLAUDE.md, OPERATOR_START_HERE.md and the execution
checklist (~3,600 lines before any code). That tax was a measured contributor to a six-hour cycle
for three trivial fixes.

**Nothing here is deleted — this is the full history, newest first.** The live board keeps the
newest entries only. Search this file when you need older context; do not read it by default.

Older still: `docs/HANDOFF_HISTORY_THROUGH_RB24.md`.
...
- **Next:** run Circuit Bridge Prompt A and Prompt B first. Accept only a compact local low-voltage bridge utility with guarded biological route, short fold-out contacts, one bridge lever, analog cause/effect, one belt slot, and no weapon/hacking drift.
- **Heads-up:** nursery pair controls state change and functional test; earlier lifecycle sheet controls final below-waist ship cultivar scale. Exact world, species name, recipe, IDs, progression, dimensions, and implementation remain provisional. No runtime, scene, recipe, save, machine, belt, WorldSpec, conveyor, or headset artifact changed.
- **Commit:** `ba89565f4566353ada4397986e0777a0ca1b74fb`
