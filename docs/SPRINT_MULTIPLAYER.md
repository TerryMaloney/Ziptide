...
| A5 | **PROGRESSION v1 SHIPPED (2026-07-10, MP100 59-62+65):** pure `PvpProgression` (MatchStatsCore kills/downs/streaks · itemized payout w/ difficulty mult, cap 150, win-gated bonuses · unlock ladder Regular→Veteran→Nightmare→mutators via profile flags · FNV daily pick, same combo for everyone per UTC day) + `PvpCareerState` additive on the profile + self-bootstrapped `PvpProgressionRuntime` (KillScored→stats; MatchEnded→RewardRouter credits + flags + career + autosave; zero patcher steps) + lobby: Veteran/Nightmare tiles LOCKED-dark until earned + the gold DAILY RUN tile. 7 tests. Sub-rows 63/64/66/67/69/70 (boards · cosmetic drops · history · rival) still open. | ✅ |
...
| B4 | Hotseat sync → (after A6) Photon live sync — ✅ HOTSEAT v1 SHIPPED (T-Dog, 2026-07-10): HOTSEAT tile on the table, END TURN passes the headset (full round = both halves then the economy turn), per-side fog re-fogs at handover, side-aware rack/builds/odds/missions, mode rides the save (tolerant H record — old saves read solo). Photon live sync still ⬜ (after A6). | 🟡 |
