...
| F3.4 | GROUNDING — kill the "everything floats" tell (decals + blob shadows) | 🟡 **commit 1 (pure + booth):** `GroundDecal` — `BlobAlpha` (soft radial contact/blob shadow) + `StainAlpha` (fBm-mottled grime) + `BakeAlpha` sheet + booth `ground_decals` subject (grey ground + a blob + a stain) + 3 tests. **commit 2 ✅ (blob shadows):** `GroundShadow` (shared dark radial decal + quad, one material across all shadows) attached at each creature's base in `ForgeCreatureVisualApplier` (parented to the host so it stays put as the body bobs) + the booth `BuildBodySubject` (so it's booth-verified) + 1 test. **commit 3 (next):** `GroundingBuilder` — stains under pipes/props + moss at wall bases, deterministic ≤40/world; + the player-rig shadow. |
