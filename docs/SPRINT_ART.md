...
| F3.1b | PRACTICALS — lanterns/sconces/street lamps + halo + light-pool decals (Terry's direct ask) | 🟡 **commit 1: the 3 fixture recipes** (`light_lantern_hang` hook+ring+caged amber glass · `light_sconce_wall` plate+bracket+half-dome (+Z against the wall) · `light_street_pole` 3m tapered pole, arm, downward head + glowing disc) — shared warm-amber practical palette (`lamplight is warm, never neon-pure`), 400-tri budget, auto-swept by library gates. **commit 2: `PracticalLight`** — halo billboard (runtime radial sprite, URP/Unlit additive One/One no-ZWrite so halos never face-fight) + light-POOL quad from the author's build-time surface point + ONE `SetLit` switch killing halo+pool+per-instance emissive (MPB — baked mats are shared) together; 2 headless tests. **✅ FIXTURE VERDICTS (run `29122261557`, CI green): lantern ✅✅ (hook, ring, cap, glowing amber body — first-try pass) · sconce ✅ (plate + drip guard + glowing half-dome) · pole ✅ (classic silhouette, amber ring glows downward — the player's view from below).** **commit 3/3 `PracticalAuthor` ✅ (Opus 4.8)** — wired into `WorldDressingBuilder.Build` after the building pass (so `__DOOR` markers exist): sconce beside ≤6 sorted doorways, poles every ~14m on the route (≤6, offset off the walk line), 2 lanterns at POI approaches; ≤14/world; each = holder + primitive Fallback + ForgeModuleLook + PracticalLight self-building halo+pool in Awake. The 2 lanterns carry the world's whole hero real-light budget (2 point lights, range 8, no shadows → 1 sun + 2 = 3 = PerfBudget cap, WARN-only). **F3.1b ✅ COMPLETE — CI green `8eccbe9` (hero-light frame cost is the runbook device risk).** |
| F3.3 | WATER — the namesake element we never had | 🟡 **commit 1 (booth-first spike):** pure `WaterSurface` (tileable ripple field — integer-wavenumber directional sines, no fBm seam; height + tangent-space normal + canonical normal-map bake) + `ZiptideWaterMesh` (flat subdivided plane, UVs tile 1/4m, tris clamped ≤8000) + booth `water_tile` subject (deep canal teal, baked ripple normal, smoothness 0.85) + 5 tests. **✅ WATER READS (run `29163148584`): deep glossy teal + directional specular ripples — canal water first try** (CI red #1 = my own over-tight normal-edge test → fixed to check ripple variation + seam-vs-interior; tiling stays proven by the continuous normal test). Slight wave BANDING = what commit 2's two-layer cross-scroll (25° apart) breaks up by design. **commit 2 ✅ (dynamics + foam):** `WaterMotion` (scroll offset + bounded two-sine swell/bob) + `WaterFoamMesh` (FLAT up-facing lacy foam band inset from the edge) + booth preview + 4 tests. **Booth verdict:** water reads deep-glossy-teal with wet specular ripples + a bright lacy white foam edge. Iteration: 2 self-inflicted test-threshold reds (fixed, traced to green) + foam went vertical-ribbon (read as a dark rim) → flat band (still culled) → **winding fix (quads faced −Y)** → foam renders. CI green `5584fac`. **commit 3 ✅:** runtime `ZiptideWater` (Awake→`Assemble`: mesh + URP/Lit water material + one scrolling normal + low-res vertex bob + foam child; prefers `ForgeBaked/water_normal`, runtime fallback; no collider) + the device normal baked via `ForgeBaker` (imported NormalMap — no BuildAndroid edit, no Sol collision) + 1 headless test. **commit 4 ✅ — F3.3 COMPLETE:** `WaterAuthor.Place` fills the shipyard BERTH with a `ZiptideWater` body (uses the existing `berthCenter`/`berthSize` — auto-aligns, no shared-data edit, no hand-authored coords), hooked in `WorldDressingBuilder.Build`; worlds without a berth get none (zero risk). Runbook 🎮: berth reads as water + y-level tune + 72fps. Canal-wide water beyond the berth = later pass (needs canal-rect data). **Two-layer scroll note:** URP/Lit detail-map keyword setup is device-unverifiable, so the runtime uses ONE scrolling normal + the bob (which breaks the banding) — a hand-written water shader is the only way to two true layers; deferred like fresnel. **Deviation (honest):** true fresnel depth-tint needs a custom shader — commit 1 uses deep color + high smoothness on stock URP/Lit instead (Quest-safe, no shader-graph); revisit only if a hand-written water shader is greenlit. |
| F3.4 | GROUNDING — kill the "everything floats" tell (decals + blob shadows) | 🟡 **commit 1 (pure + booth):** `GroundDecal` — `BlobAlpha` (soft radial contact/blob shadow) + `StainAlpha` (fBm-mottled grime) + `BakeAlpha` sheet + booth `ground_decals` subject (grey ground + a blob + a stain) + 3 tests. **commit 2 ✅ (blob shadows):** `GroundShadow` (one shared dark radial decal + quad across all shadows) attached at each creature's base in `ForgeCreatureVisualApplier` + booth. **Booth verdict (run `29172812483`): the `ground_decals` subject reads perfectly — clean soft blob shadow + irregular mottled grime stain on lit ground.** (Creature shadow invisible in the booth's dark void — a dark decal on nothing — but reads on lit ground in-game; verified by the ground subject + the attach test + CI.) One CI red = GroundShadow's static-shared mat/tex hit `ResourceDisciplineTests` → added to the exemption ledger (deliberate app-lifetime share, like ForgeMaterials). **commit 3 DEFERRED (polish):** `GroundingBuilder` stains under props + wall-base moss + player-rig shadow — device-blind placement; the headline grounding (creature blob shadows) is delivered, so this is a later polish pass, not a blocker. F3.4 CORE ✅. |
| F3.8 | MACRO VARIATION — kill the visible atlas-tiling on big surfaces | 🟡 **part ① landed:** `ForgeTexture.ComposeAlbedo` gains an optional low-freq wash (`0.90 + 0.20·fBm(u·1.7,v·1.7)`), applied in `BakeAlbedo` ONLY to `buildingModule`/`street`-tagged recipes (gated by `HasStoryTag`); default-off keeps every other asset byte-identical. 2 tests (default-off exact + wash-varies-across-island). **✅ CI+booth green (`079b9a0`).** **part ② ✅:** `ForgeModuleLook` gains `tintJitter` (±0.05, from a quantised position hash) applied as a `MaterialPropertyBlock` `_BaseColor` on the swapped-in ForgeVisual renderers — identical wall modules now differ slightly down a street, batching intact. **F3.8 COMPLETE** (device look confirms the street variation). |
| F3.5 · F3.6 · F3.7 · F3.9 | VFX · reactive world · signage · **conformance ratchet** · (creature P4 → CREATURE_QUALITY_V2) | ⬜ per the plan |
