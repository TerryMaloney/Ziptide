...
| P3.5 | **CREATURE QUALITY LADDER** (Terry's bar: "fox with a trash can lid → moving breathing xenomorph") | 🟡 **v4 SHIPPED + JUDGED (2026-07-10)** — the ladder so far: v2 smooth shading honored + capsule limbs (builder fix) → v3 bind-pose articulation (`bendDegrees`) → **v4 TEXTURE BAKE** (`fad9898`, CI green): `SyntheticRecipe` seam bakes each styled genome's albedo/normal/MSA/emissive atlas; applier + booth agree by construction; eye submesh stays live-emissive so ForgeBodyTell is untouched. **v4 verdicts (run `29089183510`): swarm_bug ✅✅ glossy amber chitin · husk_molter ✅✅ wet mossy carapace · tendril ✅ bark-speckled vines · witness_mite ✅ · light_grazer ⚠ plain (slime style too subtle) · warden ❌ near-black monolith, NO ARMS = still a bin.** → **v5 (this commit): warden gets ARMS** (3-seg mirrored chain on GaitRole.Leg — leg-ordinal phase = contralateral swing for free, 11 bones), pauldron slab split into two angled plates + bright collar, near-black visor slot, palette lifted out of the murk; Slime style gains a grime-scaled subdermal mottle layer (albedo+height) + grazer grime lifted. ⏳ verify v5 turnarounds next run |
