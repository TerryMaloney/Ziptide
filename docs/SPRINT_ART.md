...
| F3.4 | GROUNDING — kill the "everything floats" tell (decals + blob shadows) | 🟡 **commit 1 (pure + booth):** `GroundDecal` — `BlobAlpha` (soft radial contact/blob shadow) + `StainAlpha` (fBm-mottled grime) + `BakeAlpha` sheet + booth `ground_decals` subject (grey ground + a blob + a stain) + 3 tests. Next: **commit 2** blob shadow under every creature + the player rig (via ForgeCreatureVisualApplier / PlayerAvatarRig); **commit 3** `GroundingBuilder` — stains under pipes/props + moss at wall bases, deterministic ≤40/world, one shared material. |
