...
| F3.1b | PRACTICALS — lanterns/sconces/street lamps + halo + light-pool decals (Terry's direct ask) | 🟡 **commit 1: the 3 fixture recipes** (`light_lantern_hang` hook+ring+caged amber glass · `light_sconce_wall` plate+bracket+half-dome (+Z against the wall) · `light_street_pole` 3m tapered pole, arm, downward head + glowing disc) — shared warm-amber practical palette (`lamplight is warm, never neon-pure`), 400-tri budget, auto-swept by library gates. ⏳ booth verdicts; next: PracticalLight halo+pool component, then PracticalAuthor placement |
