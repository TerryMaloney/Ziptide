...
- [~] **🔧UNITY→🎮 `WorldStubGenerator` (E2) + the world buildout** — **BUILT + 11 WORLDS AUTHORED (modularity sprint, CI-green)**: fully data-driven factory (`sceneName`d layout → scene + WorldPack + per-world theme + contract + gating, regenerated every build) + `WorldLayoutLibrary`/`WorldJobLibrary` specs for **W002–W012** (Ch.1–2 complete, gating chain live, FRAGMENT_T1 at W004, Shell reveal at W012). *Remaining: full-pipeline APK audit green (see `SPRINT.md`) + Terry device-walks the arc; then W013+ = one spec per library per world.*
- [~] **⚙CI Apply `WORLD_FLOW_TEMPLATES.md`** — DONE for W002–W012 (each world's layout uses its biome archetype: underground/exterior/interior/forest/station/coastal/void). *Remaining: W013+ as they're specced.*
