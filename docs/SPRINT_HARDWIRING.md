...
| 0.4 | 🟡 **Movement + comfort layer scaffold** — *DESIGN LOCKED; currently unclaimed* | 📄 `design/COMFORT_AND_ACCESSIBILITY.md` + 🔗 `CONTROL_SCHEME.md` | **The design doc is the spec (Opus-ready): `ComfortSettings` static + preset dial table (Cozy/Standard/Bold, exact values) + diegetic console (W000 + Quarters, ArenaLobbyBoard tile idiom) + `ComfortCoverageTests` gate (motion-source manifest). Seated-mode rig offset = report-only. Standalone toggles: seated/subtitles/handedness-v1/haptic scale.** |
...
| 1.4 | ⬜ **Vertical & cavern kits + traversal** — *currently unclaimed; pure cores already shipped* | ✍️ `VERTICAL_AND_CAVERN_WORLDS.md` | Modular tunnel/chamber/shaft + mesa/platform + floating structures; ziplines, elevators, grapple, climb. Multi-level reachability audit. **Pure cores shipped first (`MultiLevelReachability` + `ZiplineCore` + `ClimbCore`, EditMode-tested); remaining work starts with cavern-KIT registration in a separate `CavernKitLibrary` (mirrors the architect's `BuildingKitLibrary`, different ids — no registry collision).** |
...
| # | Row | Design doc | Notes |
|---|-----|-----------|-------|
| 2.1 | ✅ **Ship Forge v1 (chassis + modules, ≥6 chassis)** | ✍️ `SHIP_FORGE_AND_CUSTOMIZATION.md` + 📄 `SHIP_SYSTEM.md` | **v1 SHIPPED: six chassis presets (pure, tested) + ShipRefit reproportions the berth hull live (silhouette/livery/journey-decals/nameplate/per-chassis hum).** Picasso's baked Forge meshes can supersede through the same parent (`ShipHullBuilder` invitation) without reopening the v1 row. |
| 2.2 | 🟡 **Ship loadout + wraps** — *v1 shipped; remaining seam currently unclaimed* | ✍️ `SHIP_FORGE_AND_CUSTOMIZATION.md` | **v1 SHIPPED: ShipLoadoutCore (10 modules, tradeoffs, floors, wrap-invariance by construction) + ShipLocker persistence + livery apply (the 0.3 wired-nowhere seam CLOSED).** Remaining: loadout→FlightModel feed (coordinate the ShipDefinition seam with Reasonbox). |
...
| 2.5 | 🟡 **Diegetic ship-hub home screen + world select** — *partial; currently unclaimed* | ✍️ `HOME_HUB.md` | **HANGAR surface SHIPPED (live refit tiles + holo stat readout, beside the Quarters in every berth).** Helm/world-select already existed (`ShipBoardingStation`). Remaining: cold-boot title, save slots, galaxy-map visual upgrade, garage/almanac surfaces. |
...
| # | Row | Design doc | Notes |
|---|-----|-----------|-------|
| 4.1 | ✅ **Automation / conveyor layer** | ✍️ `AUTOMATION_CONVEYORS.md` | Physical hand-built belts; items ride belts (`GamePool`); conductor mode; the unique feature. **Belt v1 vocabulary ✅ (4.1a–e: lattice/floor/hand-place/splitter/machine ports, CI-green). 4.1f PERSISTENCE ✅: player-built cells survive quit/reload (`BeltFloorSave` overlay in `WorldState.beltFloors` — ConquestSave idiom: authored layout stays canonical, only player edits stored; floorId on `BeltFloorRuntime`, sandbox=`sandbox_belt`). 4.1g GATE ✅ (`AutomationAuditRules`: floor-area/scene-cell caps BLOCK — the grid bounds buildable item count; OOB/dup-cell authoring typos caught in CI; floorId integrity for 4.1f). 4.1h CONDUCTOR ✅ (pure `BeltRoute.Trace` + `ConductorRide` — grab the teal lantern by the sandbox port, ride your line at the ore's pace, per-cell haptic ticks, release to step off; rig delta-translated, never parented). 4.1i WORLDS ✅ (belt pads are PACK DATA like mines/gardens: `BeltFloorSpawnDefinition` + `BeltPadSpawner` via a one-line announced JobDirector seam; `BeltPadLibrary` derives one "connect the mine to the depot" pad per mine-bearing world at build — W002 gets the first; grid clamped to the 4.1g budget). 4.1j RICHNESS ✅ (LAW 6 pass: belt cells are little MACHINES — rails + rollers that spin only while ore flows, splitter fork hub, depot funnel + churning agitator; pucks are per-resource deterministic shapes+colors via pure `BeltPuckStyle`, bob+yaw while riding; mine-port drill spins frantic-while-feeding + breathing piston; conductor handle is a real lantern). 4.1k BLUEPRINTS ✅ (pure `BeltBlueprint` — flood-capture the connected line, seed-anchored all-or-nothing stamp; the WAND at every dispenser: release over your line to COPY, over empty grid to STAMP, repeatable; stamps run the normal place path so persistence + caps apply). 4.1l ✅ (two hwr17 thin spots closed: SPLITTERS round-trip — pickable like belts, come back as fork-striped splitter tiles, place as splitters again; the loaded wand previews its WHOLE footprint, teal-fits/red-refuses incl. off-grid overhang). ROW 4.1 COMPLETE + richness-passed — remaining thin spots live in HANDOFF hwr18, not as an open ownership claim.** |
...
| 5.2 | 🟡 **Tidefront VR war table (B2/B3)** — *major v1 slices shipped; remaining work currently unclaimed* | 📄 `TIDEFRONT_AAA.md` | **B2 v1 SHIPPED** (orbs/lanes/build/attack/visible AI turn, zero rules in the component). **B3 v1 SHIPPED (Terry's "gulag"): every strike offers STRIKE NOW / FLY THE MISSION (+10%, underdog +15%) — sabotage pylons in the ACTUAL target world; rival attacks pause on LET IT RIDE / DEFEND (shoot down scouts). ConquestSession carries the campaign across the travel round-trip.** **Save/resume + fog-of-war + NEW WAR SHIPPED:** campaigns survive quitting (`ConquestSave` overlay in one profile flag, 4 tests), unscouted worlds are dim dots, finished wars clear + offer rematch. **FLEET RACK SHIPPED:** tap tokens to hold vessels back — strikes commit only the wave, odds card reads WAVE n/m live (pure `EstimateOdds(vesselIds)` overload). **HOTSEAT (B4) SHIPPED:** two admirals, one headset — END TURN hands the table over, fog re-fogs per side, mode survives save/quit. Remaining: Photon live sync (after A6) · space-flight defense variant (3.1 ship weapons now exist — Reasonbox seam). |
