...
| 0.4 | 🟡 **Movement + comfort layer scaffold** — *presets DESIGN LOCKED (Fable endgame #5, 2026-07-10)* | 📄 `design/COMFORT_AND_ACCESSIBILITY.md` + 🔗 `CONTROL_SCHEME.md` | **The design doc is the spec (Opus-ready): `ComfortSettings` static + preset dial table (Cozy/Standard/Bold, exact values) + diegetic console (W000 + Quarters, ArenaLobbyBoard tile idiom) + `ComfortCoverageTests` gate (motion-source manifest). Seated-mode rig offset = report-only. Standalone toggles: seated/subtitles/handedness-v1/haptic scale.** |
...
| 5.2 | 🟡 **Tidefront VR war table (B2/B3)** — *CLAIMED T-Dog/Fable (2026-07-09)* | 📄 `TIDEFRONT_AAA.md` | **B2 v1 SHIPPED** (orbs/lanes/build/attack/visible AI turn, zero rules in the component). **B3 v1 SHIPPED (Terry's "gulag"): every strike offers STRIKE NOW / FLY THE MISSION (+10%, underdog +15%) — sabotage pylons in the ACTUAL target world; rival attacks pause on LET IT RIDE / DEFEND (shoot down scouts). ConquestSession carries the campaign across the travel round-trip.** **Save/resume + fog-of-war + NEW WAR SHIPPED (2026-07-10): campaigns survive quitting (ConquestSave overlay in one profile flag, 4 tests), unscouted worlds are dim dots, finished wars clear + offer rematch.** **FLEET RACK SHIPPED (2026-07-10): tap tokens to hold vessels back — strikes commit only the wave, odds card reads WAVE n/m live (pure EstimateOdds(vesselIds) overload).** **HOTSEAT (B4) SHIPPED (2026-07-10): two admirals, one headset — END TURN hands the table over, fog re-fogs per side, mode survives save/quit.** Remaining: Photon live sync (after A6) · space-flight defense variant (3.1 ship weapons NOW EXIST — Reasonbox seam). |
