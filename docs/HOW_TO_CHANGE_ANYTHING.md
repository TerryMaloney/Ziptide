...
| **Place / tune a creature (archetypes, novels, the Warden)** | the world's builder in `WorldLayoutLibrary` — add a `CreatureZoneDef` to `kit.creatureZones` (creatureId from `Resources/Enemies`); stats/loot = the `CreatureVariantAuthor` def; NEW species = a `CreatureBehaviorBase` subclass + a factory id case in `CityBuilder.MakeCreature` | behavior attaches at build; taser/gravity already hit it (IPvpDamageable); non-lethal disable pays loot | `ZIPTIDE: CREATURE_DOWN / WARDEN mode=… / HUSK_MOLT` |
| **A biome hazard (wind/static/flood/spore/radiation)** | the world's builder in `WorldLayoutLibrary` — add a `HazardZoneDef` to `kit.hazards` (kind, center, size, strength, wind direction) | generator spawns `HazardZoneRuntime` (poll-based; slows reuse the self-healing stun path — can never latch) | `ZIPTIDE: HAZARD id=… kind=… enter/exit` |
