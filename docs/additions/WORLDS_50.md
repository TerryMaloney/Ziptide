...
| 23 | 🟡 PARTIAL (hardwiring 1.4a, reconciled): pure ride = `Traversal.ZiplineRide` (gravity/drag/push-off + hard comfort cap, tested); scene translator = `ZiplineRuntime` (self-built cable + grab handle, delta-translates the rig — never parented; ComfortVignette engages from rig motion; sag is visual-only per the core's contract). REMAINING: world placement — a patcher/WorldSpec pass stringing lines between POI pairs (one `Init(start,end)` per line). | M | WorldSpec, POI network, zipline builder | |
