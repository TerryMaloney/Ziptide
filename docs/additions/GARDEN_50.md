...
| 4 | 🟡 PARTIAL (`vvvv`): the fresh-window CLASSIFICATION (`HarvestTiming.Fresh`) shipped, but the +25% YIELD bonus is deferred — paying a bonus at ready re-baselines every existing yield + test, so it needs a balance pass that updates those together. `FreshBonus = 0` for now. | S | GardenService, PlantDefinition | |
| ~~5~~ | ✅ SHIPPED (`vvvv`): overripe decay — past the overripe onset, yield decays toward a 50% floor (never dies — all-ages). Pure + tested. *(Visual slump = a follow-up device-pass tweak in GardenPlotRuntime.)* | S | GardenService, PlotState, GardenPlotRuntime | |
