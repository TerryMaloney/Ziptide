...
deterministic/testable when given a seed.
...
Suggested systems (keep them off the Boot/world runtime — this is a separate
strategy layer): `ConquestNode/PlanetNode`, `ConquestMap`, `ConquestState`,
`ConquestRules`, `ConquestResolver`, `ConquestAction`, `ConquestSaveData`,
`ConquestAI`, `ConquestPresentation`.
