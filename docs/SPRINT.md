...
| — | — **STORY BIBLE LOCK (2026-07-06, HANDOFF gggg): Terry directive — nail the story before authoring W013+.** Narrative-only, `docs/storyboard/*` + `THE_TRANSMISSION.md`, zero code touched. — | — |
| SBL1 | Canon lock: flip `STORY_BIBLE.md` + `THE_TRANSMISSION.md` headers from PROPOSED to locked; `MASTER_BUILD_PLAN.md` §3.1 schema-drift note | ⬜ |
| SBL2 | Continuity fixes across Ch.3–7: dedupe `C4_SABLE_INTRO` (W007 vs W020), tie W038's crack-of-light forward to the Earth Approach, stage RILL's near-confession (`THE_TRANSMISSION.md` §5) at W053 | ⬜ |
| SBL3 | **The big rewrite:** `CHAPTER_8-12_ENDGAME.md` — Earth Approach upgraded from design-memo to a proper staged scene sequence; W063 Branching gets a direct RILL quote (every other signature beat has one, this — the biggest choice in the game — didn't); W064–W068 deepened so the four endings read as genuinely distinct movie beats, each explicitly landing the partner's fate, not four palette-swapped templates | ⬜ |
| SBL4 | DLC.md consistency check (light touch only — deferred content) + continuity audit appendix extending `THE_TRANSMISSION.md` §9's hooks→payoffs table | ⬜ |
| SBL5 | Close: `GAME_PLAN.md` M5 note (narrative lock complete, W013+ authoring may proceed against it) + closing HANDOFF/SPRINT summary for Picasso/Architecture/next story author | ⬜ |
