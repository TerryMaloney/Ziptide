...
| SPWN | **THE SPAWN WAVE — solved (HANDOFF uuu):** the audit checked ANY SpawnMarkerRuntime (POIs plant `poi_*` markers) → spawn checks ran at random POI props. Audit now resolves `__SPAWN_PLAYER` by name + lists ALL overlaps. 79b4437's spawn-Y/exclusion fixes retained as hygiene | 🟡 fix pushed — awaiting the verification APK |
