...
| Q2d | **Building proof**: set `buildingStyleId="toxic_tenement"` on one W002 district → APK dispatch → runbook "does it read as a place?" gate | 🟡 opt-in landed (`00aa595`) — **APK gate 🔴 blocked by the H3 spawn wave, NOT by buildings** (see HANDOFF ttt: full diagnosis, 3 named fixes, resume line). W002 itself audits clean now |
| SPWN | **🔴 THE SPAWN WAVE (breaker invoked, HANDOFF ttt):** 9 remaining SPAWN_OVERLAP_SOLID — (1) raise/per-kind POI `SpawnExclusionRadius` (props out-reach 12m; read BuildCombatCamp/BuildRuin offsets), (2) verify BuildTerrain vertices == HeightAt at spawn anchor (W004-class), (3) remove RunSpawnChecks' first-overlap `break` so ONE dispatch shows ALL blockers. One commit-pair, re-dispatch, expect green | 🔴 NEXT (before P4b) |
