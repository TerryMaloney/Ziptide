...
namespace Ziptide.Tests.PlayMode
{
    ...
    /// profile continuity, reload continuity, and zero census blockers at every destination.
    /// </summary>
    public sealed class RecoveryTravelSaveRoundTripTests
    {
        ...
        public IEnumerator NewGame_W000_ToxicCity_W000_PreservesCompositionAndDiskState()
        {
            ...
            Assert.IsTrue(ReadBootHold(FindRequired<PlayerRigPersistence>()).Held,
                "BOOT_HOLD was not armed before NEW GAME selection.");

            SelectHomeHubTileThroughXri("Tile_NEW_GAME");
            ...
            Assert.AreEqual(3, CountLogs("ZIPTIDE: SAVE_AUTOSAVE reason=travel"),
                "Each of the three real travel hops must own exactly one travel autosave.");
        ...
        private IEnumerator LoadActualBoot()
        {
            ...
            Assert.IsNotNull(FindRequired<InputActionManager>());
        }

        private void SelectHomeHubTileThroughXri(string tileName)
        {
            ...
            Assert.IsNotNull(ray, "The persistent rig has no active right XR ray.");
            Assert.AreSame(manager, ray.interactionManager,
                "The active right ray is not bound to the canonical interaction manager.");
        ...
        private PersistentIdentity CapturePersistentIdentity()
        {
            ...
            XRInteractionManager manager = FindRequired<XRInteractionManager>();
            InputActionManager input = FindRequired<InputActionManager>();
