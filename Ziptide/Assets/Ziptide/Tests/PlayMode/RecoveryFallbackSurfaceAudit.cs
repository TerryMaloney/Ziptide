...
namespace Ziptide.Tests.PlayMode
{
    ...
    /// <summary>
    /// Test-owned R1.9 material/fallback exposure layer. The existing Golden profile, runtime census,
    /// artifact guard and player-surface policy already exclude forbidden owners, prototype systems,
    /// Photon/DebugHUD artifacts and the legacy CreditsHud. This layer closes the remaining live
    /// material gap by examining every enabled renderer on an active scene object (including
    /// DontDestroyOnLoad objects) and blocking null material slots, null/error shaders and the explicit
    /// RuntimeMaterialFixer_* output contract.
    ///
    /// This is deliberately an active-runtime-surface audit, not a claim that every active renderer is
    /// currently inside the camera frustum or approved final art. Authored primitive/presentation debt
    /// remains visible in the reviewed contact sheet and is not silently relabeled as polished.
    /// </summary>
    public static class RecoveryFallbackSurfaceAudit
    {
        public const string ArtifactDirectoryName = "recovery-fallback-surfaces";
        private const string RuntimeMaterialFixerPrefix = "RuntimeMaterialFixer_";
                ...
                for (int materialIndex = 0; materialIndex < materials.Length; materialIndex++)
                {
                    ...
                    if (material == null)
                    {
                        AddFinding(
                            ...
                            "NULL_MATERIAL",
                            "NULL_SHADER",
                            "Enabled active-runtime renderer has a null material slot.");
                        continue;
                    }
                            ...
                            StringComparison.Ordinal))
                    {
                        AddFinding(
                            ...
                            materialName,
                            shaderName,
                            "RuntimeMaterialFixer replacement is exposed on an active Golden runtime surface.");
                    }
                    ...
                    if (material.shader == null)
                    {
                        AddFinding(
                            ...
                            materialName,
                            shaderName,
                            "Enabled active-runtime renderer material has no shader.");
                    }
                                 ...
                                 StringComparison.OrdinalIgnoreCase) >= 0)
                    {
                        AddFinding(
                            ...
                            materialName,
                            shaderName,
                            "Enabled active-runtime renderer is using Unity's internal error shader.");
                    }
                }
            }
        ...
        private static string BuildMarkdown(RecoveryFallbackSurfaceReport report)
        {
            ...
            text.AppendLine("- Active profile: `" + report.activeProfile + "`");
