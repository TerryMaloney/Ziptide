...
{
    /// <summary>
    /// Regression packet for the 2026-07-27 Quest evidence. These checks deliberately target shared
    /// owners and build hooks so the same failure cannot be copied into later worlds or side modes.
    /// </summary>
    public sealed class M0SystemicDeviceRegressionTests
    {
        [Test]
        public void BootPatcher_InstallsPlayerHeightSafetyAndTurnContract()
        {
        ...
        public void VehicleSafety_RejectsRoofJumpClassAndRepairsMountedEyeHeight()
        {
            ...
            string source = ReadFromProject("Ship", "Runtime", "VehicleSafetyRuntime.cs");
        ...
        private static string ReadFromProject(params string[] parts)
        {
            string path = Path.Combine(Application.dataPath, "Ziptide");
            foreach (string part in parts) path = Path.Combine(path, part);
            Assert.IsTrue(File.Exists(path), path);
