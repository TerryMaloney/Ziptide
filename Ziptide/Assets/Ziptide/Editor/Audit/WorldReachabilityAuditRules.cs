...
namespace Ziptide.Editor.Audit
{
    ...
    /// grid, floods from the player spawn via the pure <see cref="GridReachability"/>, and WARNs for any
    /// POI on a disconnected island.
    ///
    ...
    public static class WorldReachabilityAuditRules
    {
        ...
        public static void Run(SceneAuditReport report)
        {
            ...
            var reached = GridReachability.Flood(n, n, walkable, heights, sx, sy, StepTolerance);

            if (GridReachability.Count(reached) == 0) return; // spawn cell had no ground sample — bail quietly
            ...
            foreach (var poi in kit.pois)
            {
                ...
                if (!GridReachability.IsReached(reached, n, px, py))
                    report.Warning("POI_MAYBE_UNREACHABLE",
                        ...
                        "(cell " + cell.ToString("F1") + "m, step " + StepTolerance + "m). Walk it in-headset; " +
                        "if reachable, this is a coarse-grid false positive — otherwise add a graded corridor.");
            }
        }
